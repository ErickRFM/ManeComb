...
| Rama base | `main` |
| SHA base | `557115f4d62a1ba9628c04e5e3da65545001a9e5` |
| Rama de trabajo | `audit/system-authorities-phase-0-20260806` |
| Fecha | 2026-08-06 |
...
# 1. Ajuste del plan después del análisis previo

El análisis local aportado por Claude detectó correctamente:

- la diferencia entre `isCustomerAccount` y `canAccessPortal`;
- rutas `/mapa` y `/radio` dentro de Ventas;
- pérdida posible de la intención de compra por decisiones de cuenta;
- residuos visuales de demo;
- una rama y un commit locales que no fueron publicados en GitHub.

La rama local `rc-ventas-routes-demo-cards-01` y su commit `08f37ad` no están disponibles en el repositorio remoto. Por tanto, sus cambios no se consideran parte del árbol canónico y no serán copiados a ciegas. Sus hallazgos se conservan como entrada de diseño y se contrastan contra el `main` actual.

Después de ese trabajo se integró en `main` una pantalla temporal de transferencia para cuentas operativas. Esa pantalla evita el placeholder anterior, pero no representa la arquitectura final.
...
Por ello, la solución final de Fase 1 no será mantener `/mapa` y `/radio` como productos web. Será:
...
5. retirar `/mapa` y `/radio` del router de Ventas cuando todos los redirects hayan sido migrados y probados;
...
**Rama:** `audit/system-authorities-phase-0-20260806`
...
- retirar esas rutas únicamente después de que las pruebas confirmen cero consumidores.
...
Roles asignan capabilities; endpoints autorizan mediante capabilities y tenant.
...
- prueba de aislamiento multiempresa para todos los dominios protegidos;
...
Backend, Ventas, Mobile y Admin Global usan el mismo vocabulario.
...
- contratos compartidos no contienen acceso a DB;
- backend valida payloads;
- clientes no mantienen enums incompatibles;
- errores usan `code`, mensaje seguro, `requestId` y recoverability;
- `mobile/app.json` permanece como fuente de versión.
...
Plan, orden, pago, suscripción, capacidad y activación forman una cadena de estados trazable.
...
Definir ciclos de vida y un snapshot operativo compartido.
...
Mantener `communication-service` como paquete interno especializado con una interfaz pública única.
...
Distinguir fallos reales de integraciones opcionales y retirar legado con evidencia.
...
Certificar la doble barrera de Admin Global.
...
Eliminar únicamente basura demostrada y preparar dominios públicos definitivos.
...
6. CI, dependency audit y artefactos deben corresponder al SHA revisado.
