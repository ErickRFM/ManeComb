...
function platformMfaChallenge(allowedPurpose) {
  return async function (req, res, next) {
    ...
    try {
      ...
      if (allowedPurpose && payload.purpose !== allowedPurpose) {
        return res.status(401).json({ ok: false, message: "Token inválido para esta operación" });
      }
