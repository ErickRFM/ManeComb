...
async function createMongoStore() {
  ...
  // Atomico via withTransaction; CAS por activationVersion dentro de la tx; E11000 del indice unico
  // parcial ACTIVE -> already_active; sin transacciones disponibles -> fail-closed (no escribe).
  async function activateVehicleRouteAssignment(params = {}) {
    ...
    const conflictResult = (conflictReason, assignment = null, vehicle = null) => ({
      outcome: ACTIVATION_OUTCOME.CONFLICT,
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        // Patch de la asignacion (ISO -> Date para Mongo).
        ...
        // CAS dentro de la tx: solo escribe si activationVersion sigue siendo el leido (sin lost update).
        const updatedAssignment = await VehicleRouteAssignmentModel.findOneAndUpdate(
