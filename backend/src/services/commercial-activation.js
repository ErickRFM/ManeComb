const {
  TRIAL_DURATION_DAYS,
  evaluateTrialPlan
} = require("./commercial-trial-policy");

const DEFAULT_TRIAL_DAYS = TRIAL_DURATION_DAYS;
...
function buildInitialSubscriptionPeriod({ activatedAt }) {
  ...
  return { currentPeriodStart, currentPeriodEnd, paidUntil: currentPeriodEnd, nextBillingAt: currentPeriodEnd };
}
...
function evaluateTrialEligibility({ organizationId, existingOrders = [], requestedPlan, now = new Date() }) {
  ...
  const planPolicy = evaluateTrialPlan(requestedPlan);
  if (!planPolicy.allowed) return { eligible: false, code: planPolicy.code };
  ...
  return {
    eligible: true,
    code: "eligible",
    durationDays: planPolicy.durationDays,
    planId: planPolicy.planId,
    unitsLimit: planPolicy.unitsLimit
  };
}

function buildStarterFleet(order) {
  if (Array.isArray(order?.starterFleet) && order.starterFleet.length) return order.starterFleet;
...
function buildOnboardingChecklist(order) {
  if (Array.isArray(order?.onboardingChecklist) && order.onboardingChecklist.length) return order.onboardingChecklist;
  return [
    { id: "kickoff", title: "Sesión de arranque comercial", owner: "comercial", status: "pending", description: "Alinear patios, responsables y calendario operativo." },
    { id: "fleet-seed", title: "Carga inicial de flotilla", owner: "operaciones", status: "ready", description: `Base sugerida para ${Math.max(1, Number(order?.fleetSize) || 1)} unidades.` },
    { id: "admin-access", title: "Entrega de acceso administrador", owner: "soporte", status: "pending", description: "Compartir referencia comercial, accesos y siguiente paso." }
  ];
}
...
function buildLaunchSummary(order, { isTrial = false } = {}) {
  ...
  const radioSummary = order?.radioFeatureEnabled
    ...
    : "";
  if (isTrial) return `Prueba activa por ${DEFAULT_TRIAL_DAYS} días con base inicial para ${fleetCount} unidades.${radioSummary}`;
...
function buildCommercialActivationUpdate(order, mode = "active", { now = new Date() } = {}) {
  ...
  const trialDays = isTrial ? DEFAULT_TRIAL_DAYS : Math.max(0, Number(order?.trialDays) || 0);
  ...
  return {
    ...
    activatedAt,
    activationNotes: activationStatus === "active"
      ? isTrial ? `Prueba de ${trialDays} días activada automáticamente.` : "Cuenta activada automáticamente después del pago."
    ...
    launchSummary: buildLaunchSummary({ ...order, trialDays }, { isTrial }),
    status: activationStatus === "active" ? "active" : order?.paymentStatus === "paid" ? "paid" : order?.status || "new"
  };
}
