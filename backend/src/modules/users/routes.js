...
const PROFILE_FIELDS = new Set([
  "name", "email", "password", "phone", "avatarUrl", "companyName", "legalName", "taxId",
  "billingEmail", "billingAddress", "preferredMethod", "cardholderName", "cardBrand", "cardLast4",
  "cardExpMonth", "cardExpYear", "customerReference", "companyProfile", "paymentProfile",
  "operationalSchedule", "e2eePublicKey", "e2eeKeyRotatedAt"
]);
const MANAGED_USER_FIELDS = new Set([
  ...PROFILE_FIELDS, "role", "accountType", "userStatus", "status", "shift", "vehicleId"
]);
const DRIVER_ADMIN_MUTABLE_FIELDS = new Set(["operationalSchedule", "shift", "vehicleId"]);
...
function handleProfilePolicyError(res, next, error) {
  if (error instanceof ManagedUserProfilePolicyError) {
...
async function recordAudit(req, payload) {
  await req.app.locals.store.recordAppEvent?.({
    ...payload, scope: "audit", level: payload.level || "info", status: payload.status || "ok",
    userId: req.user?.id,
    metadata: { organizationId: getOrganizationId(req.user), ...payload.metadata }
  });
}

router.get("/me", authenticate, async (req, res) => {
  const profile = sanitizeProfileForViewer(req.user, await req.app.locals.store.getUserProfile(req.user.id));
...
router.patch("/me", authenticate, async (req, res, next) => {
  try {
    ...
    await recordAudit(req, {
      ...
      metadata: { targetRole: user.role, updatedFields: Object.keys(payload).filter((field) => field !== "password") }
    });
    emitOrganizationEvent(req, "user:updated", { user, organizationId: getOrganizationId(user), updatedAt: new Date().toISOString() });
    return res.json({ ok: true, data: user });
  } catch (error) {
    if (error instanceof DriverLifecycleError) return handleLifecycleError(res, next, error);
    const conflictMessages = ["El correo ya existe", "El RFC ya esta registrado"];
    error.statusCode = conflictMessages.some((msg) => error.message === msg) ? 409 : 400;
...
router.get("/", authenticate, requireOrganization, requirePermission("canViewAnalytics"), async (req, res) => {
  return res.json({ ok: true, data: await req.app.locals.store.listUsers(req.user) });
});
...
router.post("/", authenticate, requireOrganization, requirePermission("canManageUsers"), async (req, res, next) => {
  try {
    ...
    const identity = resolveManagedUserCreationIdentity(req.user, req.body);
    const payload = {
    ...
    await recordAudit(req, {
      type: "user_created", entityId: user.id, message: `Usuario ${user.email} creado`,
      metadata: { targetRole: user.role, targetUserStatus: user.userStatus }
    });
    emitOrganizationEvent(req, "users:invited", { user, organizationId: getOrganizationId(user), createdAt: new Date().toISOString() });
    await sendWelcomeEmail(user);
    return res.status(201).json({ ok: true, data: user });
  } catch (error) {
    if (error instanceof ManagedUserProfilePolicyError) return handleProfilePolicyError(res, next, error);
    if (error instanceof DriverLifecycleError) return handleLifecycleError(res, next, error);
    const conflictMessages = ["El correo ya existe", "El RFC ya esta registrado"];
    error.statusCode = conflictMessages.some((msg) => error.message === msg) ? 409 : 400;
...
router.get("/:userId/lifecycle-impact", authenticate, requireOrganization, requirePermission("canManageUsers"), async (req, res, next) => {
  try {
    const data = await previewDriverLifecycleImpact(req.app.locals.store, { organizationId: getOrganizationId(req.user), userId: req.params.userId });
...
router.post("/:userId/offboard", authenticate, requireOrganization, requirePermission("canManageUsers"), async (req, res, next) => {
  try {
    const data = await offboardDriver(req.app.locals.store, {
      actor: req.user, actorId: req.user.id, organizationId: getOrganizationId(req.user),
      reason: req.body?.reason, releaseVehicle: req.body?.releaseVehicle === true, userId: req.params.userId
    });
    await recordAudit(req, {
      type: "driver_offboarded", entityId: data.user.id, message: "Conductor dado de baja",
...
router.post("/:userId/reactivate", authenticate, requireOrganization, requirePermission("canManageUsers"), async (req, res, next) => {
  try {
    const data = await reactivateDriver(req.app.locals.store, { actor: req.user, organizationId: getOrganizationId(req.user), userId: req.params.userId });
    await recordAudit(req, { type: "driver_reactivated", entityId: data.user.id, message: "Conductor reactivado", metadata: { targetUserId: data.user.id } });
...
router.patch("/:userId", authenticate, requireOrganization, requirePermission("canManageUsers"), async (req, res, next) => {
  try {
    ...
    if (targetUser.role === "driver" && typeof requestedManagedFields.userStatus === "string" && requestedManagedFields.userStatus !== targetUser.userStatus) {
      return res.status(409).json({
        ok: false, code: "EXPLICIT_LIFECYCLE_ACTION_REQUIRED",
    ...
    if (targetUser.role === "driver") {
      ...
      if (conflictingFields.length > 0) {
        return res.status(409).json({
          ok: false, code: "DRIVER_SELF_PROFILE_AUTHORITY",
    ...
    const payload = targetUser.role === "driver" ? pickFields(req.body, DRIVER_ADMIN_MUTABLE_FIELDS) : requestedManagedFields;
    ...
    if (payload.role === "driver" && targetUser.role !== "driver") {
      return res.status(409).json({ ok: false, message: "Los conductores deben registrarse con una key de activacion" });
    }
    if (!canAccessAllTenants(req.user)) delete payload.accountType;

    let user = Object.keys(payload).length ? await req.app.locals.store.updateUser(req.params.userId, payload) : targetUser;
    ...
    if (hasVehicleTransition && requestedVehicleId !== targetUser.vehicleId) {
      vehicleTransition = requestedVehicleId
        ? await changeDriverVehicle(req.app.locals.store, { organizationId: getOrganizationId(req.user), userId: targetUser.id, vehicleId: requestedVehicleId })
        : await releaseDriverVehicle(req.app.locals.store, { organizationId: getOrganizationId(req.user), userId: targetUser.id });
      user = vehicleTransition.user;
    }
    if (!user) return res.status(404).json({ ok: false, message: "Usuario no encontrado" });

    await recordAudit(req, {
      type: "user_updated", entityId: user.id, message: `Usuario ${user.email} actualizado`,
      metadata: { targetRole: user.role, targetUserStatus: user.userStatus }
    });
    emitOrganizationEvent(req, "user:updated", { user, organizationId: getOrganizationId(user), updatedAt: new Date().toISOString() });
    ...
    if (previousStatus === "active" && currentStatus === "suspended") await sendAccountLifecycleEmail(user, "ACCOUNT_SUSPENDED");
    else if (previousStatus === "suspended" && currentStatus === "active") await sendAccountLifecycleEmail(user, "ACCOUNT_REACTIVATED");

    const emailChanged = Boolean(payload.email && String(targetUser.email || "").trim().toLowerCase() !== String(user.email || "").trim().toLowerCase());
    const passwordChanged = Boolean(payload.password && String(payload.password).trim());
    if (passwordChanged) {
    ...
    if (vehicleTransition) {
      await recordAudit(req, {
        type: requestedVehicleId ? "driver_vehicle_changed" : "driver_vehicle_released", entityId: user.id,
        message: requestedVehicleId ? "Unidad del conductor actualizada" : "Unidad del conductor liberada",
        metadata: { previousVehicleId: vehicleTransition.previousVehicle?.id || targetUser.vehicleId || null, vehicleId: vehicleTransition.vehicle?.id || null }
      });
...
router.delete("/:userId", authenticate, requireOrganization, requirePermission("canManageUsers"), async (req, res, next) => {
  if (req.user.id === req.params.userId) return res.status(400).json({ ok: false, message: "No puedes eliminar tu propia cuenta" });
  const scopedUsers = await req.app.locals.store.listUsers(req.user);
  const targetUser = scopedUsers.find((entry) => entry.id === req.params.userId);
  if (!targetUser) return res.status(404).json({ ok: false, message: "Usuario no encontrado" });
  if (targetUser.role === "owner") return res.status(409).json({ ok: false, message: "No se puede eliminar al propietario de la organización." });

  if (targetUser.role === "driver") {
    try {
      const data = await deleteDriverSafely(req.app.locals.store, {
        actorId: req.user.id, confirmation: req.body?.confirmation, organizationId: getOrganizationId(req.user),
        reason: req.body?.reason, userId: targetUser.id
      });
      await recordAudit(req, {
        type: "driver_deleted", entityId: targetUser.id, message: "Conductor eliminado lógicamente",
  ...
  if (targetUser.role === "admin" && admins.length <= 1) {
    return res.status(409).json({ ok: false, message: "No se puede eliminar al único administrador de la organización." });
  }

  let affectedVehicleIds = [];
  if (targetUser.role === "supervisor") {
    const live = await req.app.locals.store.getLiveLocations();
    affectedVehicleIds = (live.vehicles || []).filter((v) => v.supervisorId === targetUser.id).map((v) => v.id);
  }
  ...
  await recordAudit(req, {
    type: "user_deleted", entityId: req.params.userId, message: `Usuario ${targetUser.email} eliminado`,
