...
if (!JWT_SECRET) {
  throw new Error("JWT_SECRET es obligatorio y debe configurarse antes de iniciar el backend.");
}
...
const PLATFORM_MFA_ENCRYPTION_KEY = String(process.env.PLATFORM_MFA_ENCRYPTION_KEY || "").trim();
const PLATFORM_MFA_CHALLENGE_TTL = process.env.PLATFORM_MFA_CHALLENGE_TTL || "5m";
...
const LOG_LEVEL = String(process.env.LOG_LEVEL || (IS_PRODUCTION_RUNTIME ? "info" : "debug")).trim().toLowerCase();
