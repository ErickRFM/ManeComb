...
function statusTone(status: string | null | undefined) {
  ...
  if (['pending', 'in_progress', 'attention'].includes(String(status))) return styles.statusWarning;
  if (['failed', 'cancelled', 'suspended'].includes(String(status))) return styles.statusDanger;
...
export function AdminCompaniesScreen() {
  ...
  return (
    <AdminShell
      title="Empresas"
      subtitle="Vista global de clientes construida desde organizationId, propietario, plan, usuarios y unidades. Solo lectura."
    >
      <View style={styles.filterCard}>
        <View style={styles.searchRow}>
          <TextInput
            ...
            placeholder="Empresa, organización, propietario o correo"
            placeholderTextColor={palette.mutedSoft}
            style={styles.searchInput}
            value={search}
          />
          <Pressable onPress={() => applyFilters(1)} style={({ pressed }) => [styles.primaryButton, pressed && styles.pressed]}>
      ...
      {listState === 'loading' ? <StateCard title="Cargando empresas…" body="Consultando el read model global de Platform." /> : null}
      {listState === 'error' ? (
        <StateCard
          action="Reintentar"
  ...
  return (
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <Text style={[styles.statusBadge, statusTone(company.operationalStatus)]}>{company.operationalStatus}</Text>
      </View>
...
export function AdminCompanyDetailScreen({ organizationId }: { organizationId: string }) {
  ...
  const backAction = (
    <Pressable onPress={() => router.push('/admin/companies')} style={styles.secondaryButton}>
    ...
    <>
      ...
      <View style={styles.detailGrid}>
        <DetailSection title="Propietario">
          ...
          <KeyValue label="Estado" value={company.owner?.status || 'Sin estado'} />
      ...
      <DetailSection title={`Unidades (${company.vehicles.total})`} wide>
        ...
        {(company.vehicles.items || []).map((vehicle) => (
          <View key={vehicle.id} style={styles.tableRow}>
            ...
            <Text style={styles.tableCell}>{vehicle.retiredAt ? 'retired' : vehicle.status}</Text>
...
function StateCard({ title, body, tone = 'default', action, onAction }: { title: string; body: string; tone?: 'default' | 'danger'; action?: string; onAction?: () => void }) {
  return (
    <View style={[styles.stateCard, tone === 'danger' && styles.stateCardDanger]}>
...
const styles = StyleSheet.create({
  ...
  searchInput: { backgroundColor: palette.surfaceAlt, borderColor: palette.line, borderRadius: 10, borderWidth: 1, color: palette.text, flex: 1, fontFamily: Typography.body, minHeight: 44, minWidth: 240, paddingHorizontal: 14 },
  ...
  statusBadge: { borderRadius: 999, fontFamily: Typography.mono, fontSize: 9, fontWeight: '900', overflow: 'hidden', paddingHorizontal: 9, paddingVertical: 5, textTransform: 'uppercase' },
  ...
  cardFooter: { alignItems: 'center', flexDirection: 'row', gap: 10, justifyContent: 'space-between', marginTop: 14 },
  lastAccess: { color: palette.mutedSoft, flex: 1, fontFamily: Typography.body, fontSize: 9, textAlign: 'right' },
  ...
  secondaryButton: { alignItems: 'center', borderColor: palette.lineStrong, borderRadius: 9, borderWidth: 1, justifyContent: 'center', minHeight: 38, paddingHorizontal: 14 },
  ...
  metricCard: { backgroundColor: palette.card, borderColor: palette.line, borderRadius: 16, borderWidth: 1, flexGrow: 1, minWidth: 220, padding: 18, width: '23%' },
  ...
  detailSection: { backgroundColor: palette.card, borderColor: palette.line, borderRadius: 16, borderWidth: 1, flexGrow: 1, minWidth: 290, padding: 18, width: '48%' },
  ...
  keyValueRow: { alignItems: 'flex-start', flexDirection: 'row', gap: 12, justifyContent: 'space-between', paddingVertical: 7 },
  keyLabel: { color: palette.mutedSoft, fontFamily: Typography.body, fontSize: 10 },
  keyValue: { color: palette.text, flex: 1, fontFamily: Typography.body, fontSize: 11, fontWeight: '700', textAlign: 'right' },
