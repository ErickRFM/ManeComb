...
import { AdminMfaChallengeGuard } from '../components/route-guard';
import { Typography, palette } from '@/styles/theme';

export function AdminMfaVerifyScreen() {
  const { verifyMfa, recoverMfa } = useAdminStore();
  ...
  return (
    <AdminMfaChallengeGuard>
      <AdminAuthLayout
        ...
        subtitle={showRecovery ? 'Ingresa uno de tus códigos de recuperación' : 'Ingresa el código de 6 dígitos de tu app de autenticación'}
      >
        {error ? (
        ...
        {showRecovery ? (
          <>
            ...
            >
              <Text style={styles.submitText}>{isSubmitting ? 'Verificando...' : 'Verificar código de recuperación'}</Text>
            </Pressable>
          ...
          <>
            ...
            >
              <Text style={styles.submitText}>{isSubmitting ? 'Verificando...' : 'Verificar'}</Text>
            </Pressable>
...
const styles = StyleSheet.create({
  feedbackBox: { borderRadius: 12, backgroundColor: 'rgba(240, 106, 106, 0.14)', borderWidth: 1, borderColor: 'rgba(240, 106, 106, 0.46)', paddingHorizontal: 12, paddingVertical: 9 },
  ...
  input: { color: '#F8FAFC', flex: 1, fontFamily: Typography.mono, fontSize: 14, letterSpacing: 2, textAlign: 'center', minHeight: 46, paddingHorizontal: 0, paddingVertical: 0, ...(Platform.OS === 'web' ? ({ outlineStyle: 'none', outlineWidth: 0, boxShadow: 'none' } as any) : {}) },
  codeInput: { color: '#F8FAFC', flex: 1, fontFamily: Typography.mono, fontSize: 24, letterSpacing: 10, textAlign: 'center', minHeight: 46, paddingHorizontal: 0, paddingVertical: 0, ...(Platform.OS === 'web' ? ({ outlineStyle: 'none', outlineWidth: 0, boxShadow: 'none' } as any) : {}) },
  ...
  toggleLink: { color: '#FF8FB0', fontFamily: Typography.body, fontSize: 12, fontWeight: '800', textAlign: 'center' },
});
