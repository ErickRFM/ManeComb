...
type ProjectMobileRefreshResultsInput = {
...
export function projectMobileRefreshResults({
  ...
  normalizeMapData,
  toFailure,
}: ProjectMobileRefreshResultsInput) {
  ...
  results.forEach((result, index) => {
    ...
    data[key] = key === 'mapData'
      ? normalizeMapData(result.value)
      : result.value;
    fulfilledCount += 1;
  });
  ...
  for (const domain of MOBILE_RESOURCE_DOMAINS) {
    ...
    if (!result) {
      throw new Error(`Missing refresh result for ${domain}`);
    }
