...
import type { DirectMessageEnvelope } from '@shared/communication/chat-types';
...
async function buildBackupKeyMaterial(input: {
  ...
  const passwordBytes = naclUtil.decodeUTF8(`${String(input.userId).trim()}:${String(input.password)}`);
...
export async function decryptStoredChatKeyPairBackup(input: {
  ...
  if (
    ...
    throw new Error('El respaldo E2EE usa una derivación de llave no compatible.');
  }
  ...
  if (!decrypted) throw new Error('No fue posible restaurar el respaldo E2EE con esta contraseña.');
  const parsed = JSON.parse(naclUtil.encodeUTF8(decrypted)) as StoredChatKeyPair;
  if (!parsed?.publicKey || !parsed?.secretKey) throw new Error('El respaldo E2EE está incompleto.');
  return parsed;
}
...
export function isDirectChatEncryptionActive(input: {
  ...
  if (!conversation || conversation.encrypted === false) return false;
...
export function decryptDirectChatText(input: {
  ...
  if (!decryptedBytes) throw new Error('No fue posible descifrar el mensaje directo.');
  return naclUtil.encodeUTF8(decryptedBytes);
}

export const CHAT_E2EE_BACKUP_MIN_ITERATIONS = BACKUP_KDF_ITERATIONS;
